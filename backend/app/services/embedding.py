...
class EmbeddingService:
    ...
    def __init__(self):
        ...
        # Pre-built FAISS indexes (generated by scripts/build_faiss_indexes.py)
        self.track_index_path = Path("model/track_faiss.index")
        self.artist_index_path = Path("model/artist_faiss.index")
        self.faiss_mappings_path = Path("model/faiss_mappings.pkl")
    ...
    def _build_faiss_indexes(self):
        ...
        # Try to load pre-built indexes first (much faster, lower memory)
        if self._load_prebuilt_faiss_indexes():
            return

        # Fallback: build indexes from scratch (high memory usage)
        print("WARNING: Building FAISS indexes from scratch (high memory usage)")
        print("Run 'python scripts/build_faiss_indexes.py' locally to pre-build indexes")
        self._build_faiss_indexes_from_scratch()

    def _load_prebuilt_faiss_indexes(self) -> bool:
        """Load pre-built FAISS indexes. Returns True if successful."""
        if not (self.track_index_path.exists() and
                self.artist_index_path.exists() and
                self.faiss_mappings_path.exists()):
            print("Pre-built FAISS indexes not found")
            return False

        try:
            print("Loading pre-built FAISS indexes...")

            # Load indexes directly (no intermediate copies)
            self.track_faiss_index = faiss.read_index(str(self.track_index_path))
            print(f"  Loaded track index: {self.track_faiss_index.ntotal} vectors")

            self.artist_faiss_index = faiss.read_index(str(self.artist_index_path))
            print(f"  Loaded artist index: {self.artist_faiss_index.ntotal} vectors")

            # Load mappings
            with open(self.faiss_mappings_path, "rb") as f:
                mappings = pickle.load(f)
                self.track_id_to_name = mappings["track_id_to_name"]
                self.artist_id_to_name = mappings["artist_id_to_name"]

            print(f"FAISS indexes loaded: {len(self.track_id_to_name)} tracks, {len(self.artist_id_to_name)} artists")
            return True

        except Exception as e:
            print(f"Error loading pre-built indexes: {e}")
            return False

    def _build_faiss_indexes_from_scratch(self):
        """Build FAISS indexes from embeddings (fallback, high memory)"""
        import gc

        # Build track index
        if self.track_embeddings:
            self.track_id_to_name = list(self.track_embeddings.keys())
            track_embeddings_array = np.array([
                self.track_embeddings[name] for name in self.track_id_to_name
            ]).astype('float32')
            ...
            print(f"  Track index: {self.track_faiss_index.ntotal} vectors")
        ...
        if self.artist_embeddings:
            self.artist_id_to_name = list(self.artist_embeddings.keys())
            artist_embeddings_array = np.array([
                self.artist_embeddings[name] for name in self.artist_id_to_name
            ]).astype('float32')
            ...
            print(f"  Artist index: {self.artist_faiss_index.ntotal} vectors")
